  ...
  try {
    ...
    const results = trackers.map((rec) => {
      ...
      if (anyPartInHeadEnabled) {
        ...
        updated.scores.headTotalProgress = latestScoresDifference.head.overall;
      }

      if (anyPartInBodyEnabled) {
        updated.scores.bodyCurrentScore = latestScores.body.overall;
        updated.scores.bodyTotalProgress = latestScoresDifference.body.overall;
      }
    });

    res.status(200).json({ message: results });
  } catch (error) {
    addErrorLog({ functionName: "getClubTrackYou", message: error.message });
