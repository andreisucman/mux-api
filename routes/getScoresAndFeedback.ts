...
import { CategoryNameEnum, CustomRequest, FormattedRatingType, ModerationStatusEnum, ProgressType } from "types.js";
...
route.post("/", async (req: CustomRequest, res: Response, next: NextFunction) => {
  try {
    ...
    let { demographics, concerns, latestScores, latestScoresDifference, latestProgress } = userInfo;
    ...
    const promises = progressRecords.map((record) => {
      ...
      const imageObjects = record.images.map(({ mainUrl }) => ({
        part: record.part,
        url: mainUrl.url,
      }));
    ...
    if (toUpdateProgressOps.length) {
      await doWithRetries(() => db.collection("Progress").bulkWrite(toUpdateProgressOps));
    }
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      latestScoresDifference[resultsRecord.part] = resultsRecord.scoresDifference;
    ...
    latestScores.overall = Math.round(latestScoresValues.reduce((a, c) => a + c, 0) / latestScoresValues.length);
    ...
    latestScoresDifference.overall = Math.round(
      latestScoresDifferenceValues.reduce((a, c) => a + c, 0) / latestScoresDifferenceValues.length
    );

    latestProgress.overall = latestScores.overall;

    concerns = allConcerns.filter((obj, i, arr) => arr.findIndex((o) => o.name === obj.name) === i);
