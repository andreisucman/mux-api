    ...
    try {
      ...
      let taskInfo = (await doWithRetries(async () =>
        db
          .collection("Task")
          .find({
            routineId: new ObjectId(currentRoutineId),
            key: taskKey,
      ...
      const hostRoutine = (await doWithRetries(async () =>
        db.collection("Routine").findOne(
          {
            _id: new ObjectId(currentRoutineId),
            userId: new ObjectId(req.userId),
          },
      ...
      if (!hostRoutine)
        throw httpError(`${currentRoutineId} routine not found.`);

      const { allTasks: hostAllTasks } = hostRoutine;

      const relevantAllTask = hostAllTasks.find(
      ...
      const hostAllTasksWithoutTask = removeTaskFromAllTasks(
        taskKey,
        hostAllTasks
      );
      const { minDate: minHostDate, maxDate: maxHostDate } =
        getMinAndMaxRoutineDates(hostAllTasksWithoutTask);
      ...
      let { concerns: currentConcerns, allTasks: currentAllTasks } =
        targetRoutine || {};

      const targetConcerns = [
        ...new Set([...(currentConcerns || []), taskInfo.concern]),
      ];

      let targetAllTasks = combineAllTasks({
        oldAllTasks: currentAllTasks,
        newAllTasks: [updatedAllTask],
      });
      ...
      await doWithRetries(async () =>
        db.collection("Task").bulkWrite(taskUpdateOps)
      );
