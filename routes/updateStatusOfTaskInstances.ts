...
route.post("/", async (req: CustomRequest, res: Response, next: NextFunction) => {
  const { isAll, taskIds, newStatus, returnTasks, returnRoutines, timeZone }: Props = req.body;

  if (newStatus && !validTaskStatuses.includes(newStatus)) {
  ...
  try {
    ...
    if (isAll) {
      ...
      const keys = [...new Set(keyObjects.map((obj) => obj._id))].filter(Boolean);
