...
route.post("/", async (req: CustomRequest, res: Response) => {
  ...
  try {
    ...
    console.log("majorityIdentical", majorityIdentical);
    ...
    await doWithRetries({
      ...
      functionToExecute: async () => db.collection("Proof").insertOne(newProof),
    });
    ...
    if (isTaskCompleted) {
      ...
      taskUpdate.$set.nextCanStartDate = daysFrom({ days: taskInfo.restDays });
    }
    ...
    addErrorLog({ functionName: "uploadProof", message: err.message });
