...
route.post("/", async (req: CustomRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const RecipeResponseFormat = z.object({
      ...
      description: z
        .string()
        .describe(`a 2-sentence explanation of the benefits of this recipe for addressing this concern: ${concern}`),
      instruction: z
        .string()
        .describe("a step-by-step instruction on how to prepare it where each step on a new line (is separated by \n)"),
      productTypes: z
        .array(z.string())
        .describe(
    ...
    runs.push({
      ...
      responseFormat: zodResponseFormat(RecipeResponseFormat, "RecipeResponseFormat"),
      callback: () =>
        incrementProgress({
          value: 5,
          operationKey: analysisType,
