...
route.post("/", async (req: CustomRequest, res: Response, next: NextFunction) => {
  try {
    let { code, state, email, password } = req.body;
    ...
    if (code) {
      const { email, accessToken: googleAccessToken } = await getOAuthAuthenticationData({
        code,
        redirectPath,
      });
    ...
    if (userInfo) {
      ...
      if (email) {
        // login
        const timeZoneOffsetInMinutes = getTimezoneOffset(req.timeZone);
