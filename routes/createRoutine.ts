...
import { checkDateValidity, delayExecution, updateRoutineTaskCount } from "@/helpers/utils.js";
...
route.post("/", async (req: CustomRequest, res: Response, next: NextFunction) => {
  const { part, routineStartDate, taskCountMap, routineSuggestionId } = req.body;
  ...
  try {
    ...
    const routineSuggestionFilter: { [key: string]: any } = {
      userId: new ObjectId(req.userId),
      part,
    };

    if (routineSuggestionId) routineSuggestionFilter._id = new ObjectId(routineSuggestionId);

    const relevantSuggestion = (await doWithRetries(() =>
      db
        .collection("RoutineSuggestion")
        .find(routineSuggestionFilter, { projection: { tasks: 1 } })
        .sort({ createdAt: -1 })
        .next()
    )) as unknown as RoutineSuggestionType | null;

    if (!relevantSuggestion || !relevantSuggestion.tasks) {
      res.status(400).json({ error: "Bad request" });
      return;
    }

    const selectedConcernKeys = Object.keys(relevantSuggestion.tasks);
    const partConcerns = existingConcerns.filter((co) => selectedConcernKeys.includes(co.name));
    ...
    let finalTasks = relevantSuggestion.tasks;

    if (taskCountMap) {
      finalTasks = updateRoutineTaskCount(relevantSuggestion.tasks, taskCountMap);
    }
