...
route.post("/", async (req: CustomRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const recordToDelete = await doWithRetries(async () =>
      db.collection(collectionMap[collectionKey]).findOne({ _id: new ObjectId(contentId) })
    );
    ...
    switch (collectionKey) {
      case "progress":
        const { _id: userId, latestConcernScores, latestConcernScoresDifference } = userInfo;
      ...
      case "proof":
        ...
        const newActivity = activity.filter((a: DiaryActivityType) => a.contentId !== contentId);

        await doWithRetries(async () =>
          db
            .collection("Diary")
            .updateOne({ _id: new ObjectId(relevantDiaryRecord._id) }, { $set: { activity: newActivity } })
        );
        break;
    }
