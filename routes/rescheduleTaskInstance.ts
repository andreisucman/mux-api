...
route.post(
  ...
  async (req: CustomRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (!hostRoutine)
        throw httpError(`${taskInfo.routineId} routine not found.`);
      ...
      const updatedHostAllTasks = hostAllTasks
        .map((at) => {
          if (at.key === taskInfo.key) {
            const updatedRelevantAllTaskIds = at.ids.filter(
      ...
      const { minDate: minHostDate, maxDate: maxHostDate } =
        getMinAndMaxRoutineDates(updatedHostAllTasks);
      ...
      const taskUpdateOps = updatedTargetAllTasks
        ...
        .map((idObj) => ({
          updateOne: {
            ...
            update: {
              $set: { startsAt: idObj.startsAt, routineId: updateRoutineId },
            },
          },
        }));
