...
route.post("/", async (req: CustomRequest, res: Response, next: NextFunction) => {
  ...
  try {
    let validatedConcerns = userUploadedConcerns;
    ...
    if (userUploadedConcerns.length > 0) {
      validatedConcerns = await validateConcerns(userUploadedConcerns);
    }
    ...
    if (initialPartProgressImages) {
      ...
      if (Math.abs(differenceInImages) > 0) {
        ...
        console.error(`The image count doesn't match the previous record for ${req.userId}.`);
        return;
      }
    }
    ...
    await analyzeAppearance({
      ...
      userUploadedConcerns: validatedConcerns,
